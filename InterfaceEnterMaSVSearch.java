...
public class InterfaceEnterMaSVSearch {
    public InterfaceEnterMaSVSearch(ArrayList<Student> list){
        ...
        frame.setTitle("Enter MaSV Search");
        frame.setSize(300, 400);
        frame.setVisible(true);
        frame.setLocationRelativeTo(null);
        frame.setLayout(null);
        
        JLabel jLB1 = new JLabel("Enter MaSV Search"); frame.add(jLB1);
        ...
        jBT1.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                String MaSVSearch = jTF1.getText();
                boolean resultSearch = false;
                for(int i = 0; i < list.size(); i++){
                    if(list.get(i).getMaSV().equals(MaSVSearch)){
                        resultSearch = true;
                        tableModel.addRow(new Object[]{list.get(i).getMaSV(), list.get(i).getName(), list.get(i).getClassName(), list.get(i).getGpa()});
                    }                    
                }
                if(resultSearch){
